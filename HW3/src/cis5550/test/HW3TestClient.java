...
public class HW3TestClient extends GenericTest {
  ...
  void runSetup() throws Exception {
    ...
    if (!Files.exists(path)) {
      Process p = Runtime.getRuntime().exec(
        new String[] { "keytool", "-genkeypair", "-keyalg", "RSA", "-alias", "selfsigned", "-keystore", "keystore.jks", "-storepass", "secret", "-dname", 
          "\"CN=blah, OU=blubb, O=foo, L=Bar, C=US\"" }
      );
      p.waitFor();
    }
  }
  ...
  void runTests(Set<String> tests) throws Exception {
    ...
    if (tests.contains("https")) try {
      ...
      URL url = new URI("https://localhost:8443/echo/"+i).toURL();
